...
export default function PayrollScreen({ onToast }: Props) {
  ...
  return <div className="ds-page payroll-page">
    <div className="ds-page-head"><div><p className="ds-eyebrow">General</p><h1 className="ds-head-title">Payroll</h1><p className="ds-head-sub">Salary divided by required working days · weekly offs and paid holidays excluded</p></div><div className="payroll-toolbar"><label>Payroll month<input type="month" value={period} onChange={(event) => setPeriod(event.target.value)} /></label><button type="button" className="ds-ghost-btn" onClick={() => void load()} disabled={loading}><RefreshCw size={14} /> Refresh</button></div></div>
