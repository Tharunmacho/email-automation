...
def _employee_id(user: dict, requested: str | None = None) -> str:
    employee_id = requested if user.get("role") == "admin" and requested else user["id"]
    employee = users.get(employee_id)
    if not employee or not employee.active or employee.role != "staff":
...
def record_punch(payload: PunchRequest, user: dict = Depends(current_user)) -> dict:
    ...
    event, created = _conflict(lambda: service().punch(employee_id, payload, allow_recorded_time=user.get("role") == "admin"))
...
def create_adjustment(payload: AdjustmentRequest, admin: dict = Depends(require_admin)) -> dict:
...
def assign_shift(payload: ShiftAssignmentRequest, admin: dict = Depends(require_admin)) -> dict:
    _employee_id(admin, payload.employee_id)
    return {"status": "recorded", "assignment": service().assign_shift(payload, admin["id"])}


@router.post("/calendar", status_code=201)
def set_calendar_day(payload: CalendarDayRequest, admin: dict = Depends(require_admin)) -> dict:
    _employee_id(admin, payload.employee_id)
