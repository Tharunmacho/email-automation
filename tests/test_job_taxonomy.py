...
def test_a_staff_member_reaches_their_queue_and_nothing_else_by_default():
    assert pages_for("staff") == ["my-queue"]
...
def test_grants_add_and_never_subtract():
    ...
    assert set(granted) == {"my-queue", "candidates", "job-orders"}


def test_a_grant_for_a_page_that_does_not_exist_is_ignored():
    assert pages_for("staff", ["nonsense", "candidates"]) == ["candidates", "my-queue"]
